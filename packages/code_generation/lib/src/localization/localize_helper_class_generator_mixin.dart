...
extension LocalizeHelperClassGeneratorMixin on LocalizeLookupGenerator {
  ...
  void _generateClass() {
    ...
    _generated.writeln('  static String translate${i69nRoot.capitalize()}(${this.element.name} token, {Locale? locale}) => token.byLocale(locale ??  $cubitName.locale);');
    _generated.writeln('}');
  }

  void _generateExtension() {
    _generated.writeln('// MARK: Extension to $helperClassName for ${i69nRoot.capitalize()} token');
    _generated.writeln('extension $helperClassName$i69nRoot on $helperClassName {');
    _generated
        .writeln('  static String translate${i69nRoot.capitalize()}(${this.element.name} token, {Locale? locale}) => token.byLocale(locale ??  ${helperClassName}Cubit.locale);');
    _generated.writeln('}');
  }
